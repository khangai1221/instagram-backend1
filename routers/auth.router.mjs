...
router.post("/signin", async (req, res) => {
    try {
        ...
        if (!credential || !password)
            return res.status(400).send({ message: "Credential and password required" });

        const user = await UserModel.findOne({
            $or: [{ email: credential }, { phone: credential }, { username: credential }],
        });
        if (!user) return res.status(400).send({ message: "Wrong credentials!" });

        const isCorrectPassword = bcrypt.compareSync(password, user.password);
        if (!isCorrectPassword) return res.status(400).send({ message: "Wrong password!" });
